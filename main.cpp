...
void MakeNop(PVOID address, int bytes){
	DWORD d, ds;
	VirtualProtect(address, bytes, PAGE_EXECUTE_READWRITE, &d);
	memset(address, 0x90, bytes);VirtualProtect(address,bytes,d,&ds);
}

void WriteInt(PVOID address, int Value)
{
	DWORD d;
	VirtualProtect(address, 1, PAGE_EXECUTE_READWRITE, &d);
	memset(address,Value,1);
}

void MakeRet(PVOID address)
{
	DWORD d;
	VirtualProtect(address, 1, PAGE_EXECUTE_READWRITE, &d);
...
void ApplyPatches()
{
	...
	GetSystemTime(&time); 
	char StreamName[60]; 
	sprintf_s(StreamName, "CdStream%02d%i",  time.wMilliseconds,rand()%699999); 
	...
	*(BYTE*)0x869655 = 1;
	...
	MakeNop((PVOID)0x48DD7E, 5);
	MakeNop((PVOID)0x40E157, 5);
	MakeNop((PVOID)0x62A322, 36);
	MakeNop((PVOID)0x62A34E, 5);
	MakeNop((PVOID)0x40DFE4, 5);
	MakeNop((PVOID)0x601B3B, 10);
	MakeNop((PVOID)0x40E242, 5);
	MakeNop((PVOID)0x498DD2, 3);
	MakeRet((PVOID)0x568550);
	MakeRet((PVOID)0x4A68A0);
}
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
	if(DLL_PROCESS_ATTACH==fdwReason)
	{
		...
		if(tSettings.bDebug || tSettings.bPlayOnline)
		{
			...
			{
				MessageBox(0,"Incorrect gta-vc.exe version detected.\nYou must use GTA:VC 1.0 to play Vice Unleashed","VIce Unleashed Error",MB_OK);
...
void TheGameLoop()
{
	...
	{
		if((pNetGame) && GetAsyncKeyState(VK_TAB)) {
